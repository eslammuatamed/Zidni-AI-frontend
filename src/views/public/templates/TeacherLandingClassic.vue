<template>
  <template v-if="landing">
    <div class="teacher-landing__wrapper" :dir="pageDirection" :lang="activeLocale">
      ...
      <ThemeProvider
        :hero-image="selectedBg || undefined"
...
const primaryColor = computed(() => props.branding?.branding?.colors?.primary);
const secondaryColor = computed(() => props.branding?.branding?.colors?.secondary);
const accentColor = computed(() => props.branding?.branding?.colors?.accent);
const fontFamily = computed(() => props.branding?.branding?.fontFamily);

const handleBrandAvatarError = () => {
  brandAvatarError.value = true;
};

const landing = computed(() => props.landing);
...
const brandInitials = computed(() => {
  const name = (landing.value?.teacher?.displayName || landing.value?.teacher?.slug || '').trim();
  if (!name) return '?';
...
const slug = computed(() => landing.value?.teacher?.slug || '');
...
const toggleLanguage = async () => {
  locale.value = activeLocale.value === 'ar' ? 'en' : 'ar';
};
...
const editLandingRoute = computed(() => ({ name: 'teacher-landing-content' }));
const assistantLoginRoute = computed(() => ({
  name: 'assistant-login',
  query: slug.value ? { tenant: slug.value } : undefined
}));

const redirectToStudentLogin = () => {
  if (!slug.value) return;
  const target = buildTenantUrl(slug.value, '/login');
...
const assistantLinkAvailable = computed(() => Boolean(slug.value));
...
function ensureAssetUrl(value: string | null | undefined, fallback = ''): string {
  ...
  if (!candidate) return ensureString(fallback, '').trim();
  if (isBlockedExternalImage(candidate)) return ensureString(fallback, '').trim();
...
const courses = computed(() =>
  landing.value.courses.filter((course) => {
    ...
    if (COURSE_TYPE_SEGMENTS.some((segment) => normalizedType.includes(segment))) return true;
...
// SEO and JSON-LD logic is handled by the parent container
...
const landingThemeStyle = computed<Record<string, string>>(() => {
  const brandingData = (props.branding as Record<string, unknown> | undefined)?.branding as Record<string, unknown> | undefined;
  const colors = (brandingData?.colors ?? {}) as Record<string, string>;
...
/* Classic Template Specific Overrides */
.teacher-landing__modern-hero {
  flex-direction: column;
  text-align: center;
  gap: 3rem;
