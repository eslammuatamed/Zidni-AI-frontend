...
| Nav links `Pricing` / `Resources` | RouterLinks to `/pricing` and `/resources` (footer Pricing too). Figma designs for these pages exist; the **routes/views are not implemented yet**, so they currently fall through to the catch-all (`/:pathMatch(.*)*` → LandingHome) until each page task lands. | Implement `/pricing` and `/resources` views + routes. Remaining footer placeholders (`#`): Documentation, Case Studies, Community, About Us, Careers. Footer social buttons have no destinations specified in the design (`#`). |
